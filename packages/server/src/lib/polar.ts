...
export async function getCustomerDebugInfo(customerExternalId: string) {
  try {
    const customerState = await polar.customers.getStateExternal({
      externalId: customerExternalId,
    });
...
export async function addCreditsManual({
  ...
  await polar.events.ingest({
    events: [
      {
        name: "nyxcode_purchase",
        externalId: `manual-credits-${Date.now()}`,
        externalCustomerId,
        metadata: { credits },
      },
    ],
  ...
  return { success: true, credits, customerId: externalCustomerId };
}
